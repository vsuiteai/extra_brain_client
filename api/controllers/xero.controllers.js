...
export const getXeroConnectionStatus = async (req, reply) => {
  try {
    const userId = req.user?.id;
    if (!userId) return reply.code(401).send({ error: 'Unauthorized' });
    ...
    req.log.error(e, 'Xero getXeroConnectionStatus error');
    return reply.code(500).send({ error: 'Failed to check Xero connection status', details: e.message });
  }
};
